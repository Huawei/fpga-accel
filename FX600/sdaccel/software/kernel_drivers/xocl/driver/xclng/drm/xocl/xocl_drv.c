...
static int probe_feature_rom(struct drm_xocl_dev *xdev)
{
	...
	// Magic number check
	if (val != 0x786e6c78) {
      if (xdev->ddev->pdev->vendor == 0x19e5 && (xdev->ddev->pdev->device == 0xd515 || xdev->ddev->pdev->device == 0xf000)) {
        ...
        strncpy(xdev->header.VBNVName, "huawei_vu9p_dynamic_f100_vc_5_1", 35);
        ...
        printk(KERN_INFO "XOCL: ddr is : %d\n", ddr);
        printk(KERN_INFO "XOCL: xdev->header.DDRChannelCount is : %d\n", xdev->header.DDRChannelCount);
