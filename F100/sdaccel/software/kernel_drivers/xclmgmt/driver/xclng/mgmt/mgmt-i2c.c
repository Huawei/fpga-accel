...
int mgmt_xiic_init(struct xclmgmt_dev *lro)
{
	...
	/* skip I2C for twitch board */
	if (lro->pci_dev->device == 0x6b0f || lro->pci_dev->device == 0x6a8f 
        || lro->pci_dev->device == 0xd513 || lro->pci_dev->device == 0xd518 || lro->pci_dev->device == 0xd516)
		return 0;
	...
	i2c->base = (void __iomem *)lro->bar[XCLMGMT_MAIN_BAR] + AXI_I2C_BASE;

	lro->i2c = i2c;
	i2c->adap = xiic_adapter;
	snprintf(i2c->adap.name, sizeof(i2c->adap.name) - 1, 
		"xclmgmt%d-i2c-%s", lro->instance,
		dev_name(&lro->pci_dev->dev));
	i2c_set_adapdata(&i2c->adap, i2c);

	i2c->adap.dev.parent = &lro->pci_dev->dev;
	...
	if (ret) {
		mgmt_err(lro, "add i2c adapter failed: %d\n", ret);
		goto failed;
	}
	...
	if (ret < 0) {
		mgmt_err(lro, "read config fan failed");
		return 0;
	}
	pwm_config_reg = data.byte;

	mgmt_info(lro, "PWM_CONFIG_REG is :%x", pwm_config_reg);
	...
	if(tach_value > MAX_TACH_THRESHOLD){
		mgmt_info(lro, "Detected fan's RPM is too slow %d",
			tach_value);
		...
		mgmt_info(lro, "Fan initialized, tach=0x%x",
			tach_value);
...
cont:
	lro->lm63 = i2c_new_device(&i2c->adap, &lm96163_board_info);
	if (!lro->lm63) {
		mgmt_err(lro, "add lm96163 failed \n");
	}
...
void mgmt_xiic_fini(struct xclmgmt_dev *lro)
{
	...
	if (lro->lm63) {
		i2c_unregister_device(lro->lm63);
	}
	/* remove adapter & data */
