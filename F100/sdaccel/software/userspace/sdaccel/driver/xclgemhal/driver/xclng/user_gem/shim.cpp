...
static const std::map<std::string, std::string> deviceOld2NewNameMap = {
    std::pair<std::string, std::string>("huawei:vu9p_dynamic:f100_vc:5.1", "huawei_vu9p_dynamic_f100_vc_5_1"),
    std::pair<std::string, std::string>("xilinx:huawei-vu5p:2ddr-dynamic:5.1", "xilinx_huawei-vu5p_2ddr-dynamic_5_1"),
    std::pair<std::string, std::string>("huawei:vu9p_dynamic:fp1:5.0", "huawei_vu9p_dynamic_fp1_5_0")
};
                        ...
                        size_t size, size_t offset)
{
    ...
    int ret=ioctl(mUserHandle, DRM_IOCTL_XOCL_SYNC_BO, &syncInfo);
    return ret;
...
//Device ddr init for F100 version
int XOCLShim::cddrInit()
{     
    int ret;
    int ddrinit_tm;
    unsigned int cddr_reg = 0x0;
    
    if (mLogStream.is_open()) {
        mLogStream << __func__ << ", " << std::this_thread::get_id() << std::endl;
    }

    /*read and check init statue is done or not*/
    ret = pcieBarRead(SHIM_USER_BAR, CDDR_INIT_OFFSET, &cddr_reg, 4);
    if (ret)
    {
        std::cout << "ERROR: Could not read ddr init reg\n";
        return ret;
    }
    ...
    {         
        cddr_reg = 0x0;
        ret = pcieBarWrite(SHIM_USER_BAR, CDDR_INIT_OFFSET, &cddr_reg, 4);
        std::cout << "  DDR initial done flag is clear!"<< std::endl;
        if (ret)
        {
            std::cout << "ERROR: Could not write 0 to ddr init reg\n";
            return ret;
        }    
    }
    ...
    if(0 == cddr_reg)
    {
        std::cout << "Start initialing DDR MEMORY\n";
        cddr_reg = 0x0;
        ret = pcieBarWrite(SHIM_USER_BAR, CDDR_INIT_OFFSET, &cddr_reg, 4);
        if (ret)
        {
        ...
        if (ret)
        {
            std::cout << "ERROR: Could not write 1 to ddr init reg\n";
            return ret;
        }
    }
  
    /*wait for initing complete*/
    ddrinit_tm = 0;
    sleep(1);
    ret = pcieBarRead(SHIM_USER_BAR, CDDR_INIT_OFFSET, &cddr_reg, 4);    
    if (ret)
    ...
    while (0 == cddr_reg)
    {
        if (ddrinit_tm > CDDR_INIT_TMOUT)
        {
            std::cout << " ERROR : DDR initialized time out!  "<< std::endl;
            ret = -1;
            break;
        }
        ...
        if (ret)
        {
            std::cout << "ERROR: Could not read ddr init reg , access time is %d !"<< ddrinit_tm << std::endl;
            break;
        }
    ...
    std::cout <<"\n";
    std::cout << "DDR initial done!"<<std::endl;
    return ret;
...
{
    xocl::XOCLShim *drv = xocl::XOCLShim::handleCheck(handle);
    int ret = drv ? drv->xclSyncBO(boHandle, dir, size, offset) : -ENODEV;
...
int xclCddrInit(xclDeviceHandle handle)
{
    xocl::XOCLShim *drv = xocl::XOCLShim::handleCheck(handle);
    if (!drv) {
        return -1;
