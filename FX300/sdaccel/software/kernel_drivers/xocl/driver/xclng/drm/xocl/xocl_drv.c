...
#define XOCL_DRIVER_PATCHLEVEL  6
...
static int probe_feature_rom(struct drm_xocl_dev *xdev)
{
	...
	// Magic number check
	if (val != 0x786e6c78) {
      if (xdev->ddev->pdev->vendor == 0x19e5 && (xdev->ddev->pdev->device == 0xd517 || xdev->ddev->pdev->device == 0xf000)) {
        printk(KERN_INFO "XOCL: Found HW VU5P Device without featureROM\n");
        ...
        strncpy(xdev->header.FPGAPartName, "HW VU5P", 8);
        memset(xdev->header.VBNVName, 0, sizeof(xdev->header.VBNVName));
        strncpy(xdev->header.VBNVName, "xilinx_huawei-vu5p_2ddr-dynamic_5_1", 35);
        ...
        xdev->header.DDRChannelCount = 2;
        xdev->header.DDRChannelSize = 16;
        xdev->header.FeatureBitMap = 0x0;
        printk(KERN_INFO "XOCL: Enabling HW dynamic 5.1 DSA\n");
