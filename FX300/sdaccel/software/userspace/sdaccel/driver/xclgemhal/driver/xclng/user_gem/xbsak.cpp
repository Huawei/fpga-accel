...
#endif
    case xcldev::CDDRINIT:
        result = deviceVec[index]->ddrinit();
        break;
...
void xcldev::printHelp(const std::string& exe)
{
    ...
    std::cout << "Run Devices query device[0] \n";
    std::cout << "  " << exe << " query\n";
    std::cout << "Run Devices query device[1] \n";
    std::cout << "  " << exe << " query -d 1\n";
    std::cout << "Run DDR init \n";
    std::cout << "  " << exe << " ddrinit\n";
}
